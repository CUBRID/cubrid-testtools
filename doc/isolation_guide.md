...
```
For more instructions on CTP, please see  https://github.com/CUBRID/cubrid-testtools/blob/develop/CTP/README.md.   
...
## 2.4 Execute test on multi-nodes
...
```
runone ~/cubrid-testcases/isolation/_01_ReadCommitted/authorization/method/add_user_01.ctl 300
```
...
#default value of ssh parameters
default.ssh.port=22
default.ssh.user=isolation
default.ssh.pwd=PASSWORD

#default value of cubrid configurations
...
#The scenario paths of all test nodes must be consistent.
...
2. Send test message
    1. To execute an isolation test with release build
    ```bash
    ...
    ```
    2. To execute an isolation_debug test with debug build
    ```bash 
...
When you sent a test message, the consumer will catch the test massage and starts the test. You can login to the `controller node` to see the execution log.   
```bash
$ tail -f nohup.out
++ cat /home/controller/CTP/common/sched/result/consumerMsg.info
++ grep MSG_ID
...
Do you accept above message [Y/N]:
```
For code coverage test, it doesn't show the progress in the QA homepage.  You can log in to the controller node to see the execution log and the detailed logs of each node in the `$CTP_HOME/result/isolation/current_runtime_logs`. After the test finished, you can find the result in the `code coverage` node. See [verify code coverage testing result](#verify-code-coverage-testing-result) for detail.   
...
* Regression issue
http://jira.cubrid.org/browse/CBRD-22630
* Compile issue
...
* The current isolation test case is located in the https://github.com/CUBRID/cubrid-testcases repository. In the isolation directory, test cases are stored in four different combination of isolation levels. Each isolation level directory is further divided into multiple catalogs according to the purpose of the test. 
    ```
    $ cd cubrid-testcases/isolation
    $ ls
    _01_ReadCommitted  _02_RepeatableRead  _04_RepeatableRead_ReadCommitted  _05_ReadCommitted_RepeatableRead  config
...
* The suffix of an isolation test case is `.ctl`. The expected answer file has the same name as the test case and the suffix is `.answer[id]`. The answer files should be in the `answer` folder, and the `answer` folder and test cases are in the same path.    
   ...
   ```
* A test case can have multiple answer files, distinguished by `id` of `.answer[id]` like `insert_update_07_1.answer` and `insert_update_07_1.answer1`. Because in some cases, we don't know which transaction gets the lock first so we save both results as answer if they are expected.   
...
Perform the transaction mix scenario. 
	...
	* `MC: wait until C1 ready;`: It means you expect C1 processed normally. If C1 is blocked, the test case will fail. Other clients will be executed after C1 ready. In above test case, `C1: update xoo set a=a where a=5;` and `C2: select * from xoo where a=5;`, if there is no `MC: wait until C1 ready;`, the 'C2 select' may executed before 'C1 update', that's not our expectation.
...
### Execute isolation test case by csql
